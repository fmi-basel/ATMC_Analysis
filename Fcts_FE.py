...
def extract_features(ome_zarrs_dict, ome_zarrs_df, table_name, label_name, pyramid_level, source, folder, analysis_dir, barcodes, experiment_setup, thresholds, stainings, result_file_name, radius_multiplier, sigma_skeleton, quantiles_to_calc):
    ...
    for i, bc in enumerate(tqdm(ome_zarrs_dict.keys(), desc="Barcodes")):
        ...
        for well_idx, well in enumerate(tqdm(well_names, desc=f"Wells ({bc})", leave=False)):
            ...
            for row_idx, row in enumerate(table.itertuples()):
                ...
                for channel in range(img.shape[0]):
                    images["C0"+str(channel+1)] = img[channel,0]

                
                # Load mask into MASK image
                images["Mask"] = mask[label_name][0]
                images["Mask"][images["Mask"] != int(row_idx)+1] = 0 #Remove other labels than current organoid
                ...
                if np.max(measure.label(images["Mask"].astype(bool))) == 1:
                    ...
                    df.loc[OID, "Barcode"] = "-".join(OID.split("-")[:-2])
                    df.loc[OID, "Well"] = OID.split("-")[-2]
                    df.loc[OID, "Object"] = OID.split("-")[-1]
                    ...
                    df.loc[OID, "Cell_line"] = experiment_setup["-".join(OID.split("-")[:-2])][OID.split("-")[-2]][2]
                    df.loc[OID, "Other"] = experiment_setup["-".join(OID.split("-")[:-2])][OID.split("-")[-2]][3]
        ...
        df.to_csv(save_path)
        print("Saved %s results as %s."%(bc, save_path))
